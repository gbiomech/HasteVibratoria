...
def cogveap(COP, freq, mass, height, show=False, ax=None):
    """COGv estimation using COP data based on the inverted pendulum model.
...
def _plot(COP, COGv, freq, ax):
    ...
    try:
        ...
        ax.plot(time, COP, color=[0, 0, 1, .8], lw=2, label='$CoP_{ap}$')
        ax.plot(time, COGv, color=[1, 0, 0, .8], lw=2, label='$CoG_{ap}$')
